...
st.markdown("""
    ...
    .close-btn {
        border: 2px solid #EF4444; color: #EF4444; background-color: white;
    }
    .plan-card {
...
with st.sidebar:
    ...
    # Input de Código
    codigo_acceso = st.text_input("Ingresa tu Código:", type="password", placeholder="Ej: PRUEBA1")
    ...
    if codigo_acceso:
        ...
        if usuarios_db:
            ...
            if usuario_encontrado:
                ...
                # Botón de Salir (Limpiar campo)
                if st.button("🔓 Cerrar Sesión"):
                    # Esto fuerza recargar la página vaciando el input visualmente
                    st.rerun()
            else:
...
if ver_precios and not es_pro:
    ...
    with cp:
        ...
        # Aquí está la corrección de los datos
        st.markdown("""
        **Banco:** ITAÚ  
    ...
    with cw:
        ...
        st.markdown(f'<a href="{link}" target="_blank"><button style="background-color:#25D366;color:white;border:none;padding:10px;border-radius:5px;width:100%;font-weight:bold;">📲 Enviar Comprobante</button></a>', unsafe_allow_html=True)
    
    st.divider()
    
    # --- BOTÓN PARA VOLVER A GRATIS ---
    # Este botón soluciona el problema de quedarse atrapado en la pantalla de precios
    if st.button("❌ Volver a la App (Modo Gratis)", on_click=cerrar_planes):
        pass # La función cerrar_planes ya maneja el estado

# =======================================================
# === APP PRINCIPAL ===
# =======================================================

# Solo mostramos la App si NO estamos viendo precios (para limpiar la pantalla)
if not ver_precios or es_pro:
